...
app.get('/api/encuesta/establecimientos',async(req,res)=>{try{const p=await getPool();const r=await p.request().query('SELECT DISTINCT r.establecimiento,ISNULL(e.nombre,r.establecimiento) AS nombre FROM dbo.pos_encuesta_respuesta r LEFT JOIN dbo.core_establecimiento e ON e.establecimiento=r.establecimiento WHERE almacen LIKE "%ARDP%" AND almacen NOT IN("ARDP-0002", "ARDP-0007", "ARDP-0155")');res.json(r.recordset);}catch(e){res.status(500).json({error:e.message});}});
app.get('/api/establecimientos',async(req,res)=>{try{const p=await getPool();const result=await p.request().query('SELECT establecimiento,nombre FROM dbo.core_establecimiento WHERE almacen LIKE "%ARDP%" AND almacen NOT IN("ARDP-0002", "ARDP-0007", "ARDP-0155"');res.json(result.recordset);}catch(e){res.status(500).json({error:e.message});}});
